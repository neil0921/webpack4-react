...
function LayoutView({dispatch, localeLanguage}) {
  ...
  const locale = localeLanguage.lang === 'en' ? enUS : zhCN, lang = localeLanguage.package;
  ...
  return (
    <ConfigProvider locale={locale}>
      <div className={flag ? 'title-red' : 'title-green'}>
        ...
        <Button type='primary' onClick={changeLocal}>{localeLanguage.lang === 'en' ?'中文':'English'}</Button>
        <img src={logo} alt=""/>
      </div>
    </ConfigProvider>
  )
